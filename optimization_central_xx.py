...
def compute(nodes, options, week, net_data, params):
    ...
    print(sum(nodes[n]["pv_power"][:,idx][t] for n in nodes for t in time_steps), 'PV')
    print(sum(nodes[n]["heat"][:,idx][t] for n in nodes for t in time_steps), 'wärme')
    print(sum(nodes[n]["elec"][:,idx][t] for n in nodes for t in time_steps), 'strom')
    print(sum(nodes[n]["dhw"][:,idx][t] for n in nodes for t in time_steps), 'dhw')
    ...
    for n in nodes:
        ...
        # grid losses VARIABLES
    if options["grid"]:
            P_loss = model.addVars(net_data["nodeLines"], time_steps, vtype="C", lb=-10000, name="powerLoss")
            P_loss_Ges = model.addVar(vtype="C", lb=-gp.GRB.INFINITY, name="obj")
            I_quad = model.addVars(net_data["nodeLines"], time_steps, vtype="C", lb=-10000, name="Current_quad")
            Res = model.addVars(net_data["nodeLines"], vtype="C", lb=-10000, name="Resistance")
            Resqua = model.addVars(net_data["nodeLines"], vtype="C", lb=-10000, name="Resistancequad")
            V_HIlF = model.addVars(net_data["nodeLines"], time_steps, vtype="C", lb=-10000, name="VHilfe")

            V_quad = {}
            for i in range(1, 228):
                V_quad[i] = {}
                for t in time_steps:
                    V_quad[i][t] = model.addVar(vtype="C", name="VoltageQuad" + str(i) + "_t" + str(t))
    ...
    P_inj_Geb_Ges = model.addVar(vtype="C", name="dd")
    P_load_Geb_Ges = model.addVar(vtype="C", name="ff")
...
#%% EV CONSTRAINTS CONSTRAINTS
    ...
    model.addConstr(P_inj_Geb_Ges == sum(residual["feed"][t] for t in time_steps))
    model.addConstr(P_load_Geb_Ges == sum(residual["power"][t] for t in time_steps))
    ...
    for t in time_steps:
        ...
        if options["grid"]:
            # Quartiersbilanz mit Gesamtverlusten
            model.addConstr(residual["feed"][t] + power["from_grid"][t] - sum(P_loss[n, m, t] for [n, m] in net_data["nodeLines"])== residual["power"][t] + power["to_grid"][t])
            #
        else:
    ...
    if options["grid"]:
        ...
        for node in net_data["gridnodes"]:
            ...
            for t in time_steps:
                ...
                if node in net_data["net_nodes"]["trafo"]:
                    ...
                    model.addConstr(powerLine.sum(node,'*',t) - powerLine.sum('*',node,t) ==
                                    power_net["Inj"][node][t] - power_net["Load"][node][t]- P_loss.sum('*', node, t), name="node balance_"+str(node))
        #
        for [n, m] in net_data["nodeLines"]:
            if [n, m] == [1, 0]:
                ...
                model.addConstr(Res[n, m] == net_data["res"][n, m] * net_data["length"][n, m],name="1")  # Berechnung Widerstand = spez. Widerstand * Länge
                model.addConstr(Resqua[n, m] == net_data["resQua"][n, m], name="2")  # quadratischer Widerstand
                ...
                for t in time_steps:
                    model.addConstr(P_loss[n, m, t] == Res[n, m] * I_quad[n, m, t] / 1000, name="3") # Berechnung Verlustleistung

                    model.addConstr(V_quad[m][t] == V_quad[n][t] - 2 * Res[n, m] * powerLine[n, m, t] / 1000 + V_HIlF[n, m, t] / 1000000, name="4") # quadratische Spannungsgröße berechnen

                    model.addConstr(V_HIlF[n, m, t] == ((Resqua[n, m]) * I_quad[n, m, t]), name="5") # Hilfsgröße für quadr. Spannungsgröße


                    # konvexe Relaxation
                    model.addConstr(V_quad[m][t] * I_quad[n, m, t] >= (powerLine[n, m, t]) ** 2, name="6")


        for t in time_steps:
            model.addConstr(V_quad[1][t] == (0.400) ** 2, name="9") # Startwert Spannung
        ...
        for node in net_data["gridnodes"]:
            if node == 0:
                ...
                for t in time_steps:
                    model.addConstr(V_quad[node][t] <= (0.440) ** 2, name="7") # maximale Spannung
                    model.addConstr(V_quad[node][t] >= (0.360) ** 2, name="8") # minimale Spannung

        # Gesamtverluste
        model.addConstr(P_loss_Ges == sum(P_loss[n, m, t] for [n, m] in net_data["nodeLines"] for t in time_steps))
    ...
    if options["district"]:
        ...
        model.addConstr(
            ...
            (res_dom["power"][n][t] * params["eco"]["elec_price"] - res_dom["feed"][n][t] * params["eco"]["eeg_pv"]) for t in
            time_steps for n in nodes))
        # Emissions
        model.addConstr(
